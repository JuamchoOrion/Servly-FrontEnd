...
export class CategoryListComponent implements OnInit, OnDestroy {
  ...
  deleteCategory(category: Category): void {
    if (confirm(this.i18n.translate('categories.confirmDelete'))) {
      this.productService.deleteCategory(category.id)
        .pipe(takeUntil(this.destroy$))
        .subscribe({
          next: () => {
            this.successMessage = this.i18n.translate('categories.deletedSuccess');
            this.loadCategories();
            this.cdr.markForCheck();
          },
          error: (error: any) => {
            this.errorMessage = this.i18n.translate('categories.errors.deleteFailed');
            this.cdr.markForCheck();
          }
        });
    }
